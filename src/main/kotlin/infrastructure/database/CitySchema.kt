...
class CityService(private val connection: Connection) {
    companion object {
        ...
        private const val SELECT_ALL_CITY = "SELECT name, population FROM cities"
    ...
    suspend fun readAll(): List<City> = withContext(Dispatchers.IO) {
        val statement = connection.prepareStatement(SELECT_ALL_CITY)
        val resultSet = statement.executeQuery()
